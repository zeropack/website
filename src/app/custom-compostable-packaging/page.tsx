...
const morePackagingOptions = [
  ...
  {
    t: "Garment bags",
    d: "Apparel workflows can combine branded mailers with internal garment protection where the specification is suitable.",
    image: imgGarmentBags,
  },
  {
    t: "Grocery and carry bags",
    d: "Custom carry-bag formats can be explored for retail, grocery, events and campaign use, subject to the required specification.",
    image: imgGroceryBags,
  },
  {
    t: "Stand-up pouches",
    d: "Resealable pouch formats may be available for suitable projects. Material, product-contact and certification requirements are confirmed before quoting.",
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const description =
    market === "uk"
      ? "Explore custom compostable packaging for UK brands beyond mailers, including garment bags, shopping bags and other made-to-order formats subject to specification."
      : market === "au"
        ? "Explore custom compostable packaging for Australian brands beyond mailers, including shopping bags, garment bags and other made-to-order formats."
        : "Broader custom compostable packaging beyond mailers: shopping bags, garment bags and campaign packaging — quoted to order.";

  return buildMarketPageMetadata({ market, title: "Custom Compostable Packaging | Zero Pack", description, path });
}
        ...
        <div className="mx-auto grid max-w-6xl gap-10 px-4 sm:px-6 lg:grid-cols-2 lg:items-start lg:gap-12">
          <div>
            ...
            <p className="mt-5 text-lg text-charcoal/75">
              ...
              are our primary format for ecommerce shipping, we can explore a wider range of made-to-order compostable
              packaging where the product, use case and required certification are a suitable fit.
            </p>
            {market === "uk" ? (
              <p className="mt-5 rounded-xl border border-slate-200/70 bg-stone p-4 text-sm leading-relaxed text-charcoal/70">
                For UK projects, we confirm the relevant certification, production requirements and freight assumptions
                for the exact format before it is quoted.
              </p>
            ) : null}
          ...
          <CustomMailerCarousel slides={packagingSlidesForCarousel()} variant="climate" permanentCaption />
        </div>
      </section>
